...
func _ready() -> void:
	...
	class_func_tree.multi_selected.connect(
		func(item: TreeItem, col: int, selected: bool): save_selection(selected, item)
	)
...
func collect_all_class_functions(script: Script) -> Dictionary[StringName, PackedStringArray]:
	...
	var class_to_functions: Dictionary[StringName, PackedStringArray] = collect_super_class_functions(
		script.get_base_script(), existing_funcs
	)
	var native_class_to_functions: Dictionary[StringName, PackedStringArray] = collect_native_class_functions(
		script.get_instance_base_type(), existing_funcs
	)

	return native_class_to_functions.merged(class_to_functions)


func collect_super_class_functions(
	base_script: Script, existing_funcs: Dictionary[String, bool]
...
func collect_native_class_functions(
	native_class: StringName, existing_funcs: Dictionary[String, bool]
) -> Dictionary[StringName, PackedStringArray]:
	...
	for super_native_class: StringName in super_native_classes:
		var functions: PackedStringArray = collect_class_functions(
			super_native_class, existing_funcs
		)
		if functions.is_empty():
			continue
...
func collect_script_functions(
	super_class: Script, existing_funcs: Dictionary[String, bool]
) -> PackedStringArray:
	var functions: PackedStringArray = []
