...
const DEFAULT_ORDER: PackedStringArray = [
	ENGINE_FUNCS, FUNCS, SIGNALS, EXPORTED, PROPERTIES, CONSTANTS, CLASSES
]
#endregion
...
func init_outline_order():
	...
	outline_type.add_to_outline = func():
		add_to_outline_if_selected(
			signal_btn, func(): add_to_outline(outline_cache.signals, signal_icon, &"signal")
		)
	outline_type_order.append(outline_type)
	...
	outline_type.add_to_outline = func():
		add_to_outline_if_selected(
			property_btn, func(): add_to_outline(outline_cache.properties, property_icon, &"var")
		)
	outline_type_order.append(outline_type)
	...
	outline_type.add_to_outline = func():
		add_to_outline_if_selected(
			class_btn, func(): add_to_outline(outline_cache.classes, class_icon, &"class")
		)
	outline_type_order.append(outline_type)
...
func update_outline_button_order():
	var all_buttons: Array[Button] = [
		engine_func_btn, func_btn, signal_btn, export_btn, property_btn, class_btn, constant_btn
	]
...
func add_to_outline(
	items: Array[String], icon: Texture2D, type: StringName, modifier: StringName = &""
):
	add_to_outline_ext(items, func(str: String): return icon, type, modifier)


func add_to_outline_ext(
	items: Array[String], icon_callable: Callable, type: StringName, modifier: StringName = &""
):
	...
	if is_sorted():
		...
		items.sort_custom(
			func(str1: String, str2: String): return str1.naturalnocasecmp_to(str2) < 0
		)
	...
	for item: String in items:
		if text.is_empty() || text.is_subsequence_ofn(item):
			...
			var dict: Dictionary[StringName, StringName] = {TYPE: type, &"modifier": modifier}
