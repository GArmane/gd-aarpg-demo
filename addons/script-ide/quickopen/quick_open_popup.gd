...
const ORDER: Dictionary[StringName, int] = {&"PackedScene": 0, &"GDScript": 1, &"Object": 2}

const Plugin := preload("uid://bc0b5v66xdidn")

enum Category { ALL = 0, SCENES = 1, GDSCRIPTS = 2, RESOURCES = 3 }
...
func build_file_cache_dir(dir: EditorFileSystemDirectory):
	...
	for index: int in dir.get_file_count():
		...
		if file_name.length() + 6 != file.length():
			file_structure = (
				SEPARATOR + STRUCTURE_START + file.substr(6, last_delimiter - 6) + STRUCTURE_END
			)
	...
	for file_data: FileData in files:
		...
		if filter_text.is_empty() || filter_text.is_subsequence_ofn(file):
			var icon: Texture2D = EditorInterface.get_base_control().get_theme_icon(
				file_data.file_type, &"EditorIcons"
			)
