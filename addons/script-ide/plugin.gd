...
const OPEN_QUICK_SEARCH_POPUP_GDSCRIPTS: StringName = (
	SCRIPT_IDE + &"open_quick_search_popup_scripts"
)
## Editor setting for the 'Open Quick Search Popup (Resources)' shortcut
const OPEN_QUICK_SEARCH_POPUP_RESOURCES: StringName = (
	SCRIPT_IDE + &"open_quick_search_popup_resources"
)
...
func _enter_tree() -> void:
	...
	script_editor_split_container = find_or_null(
		script_editor.find_children("*", "HSplitContainer", true, false)
	)
	...
	outline_container.is_hide_private_members = get_setting(
		HIDE_PRIVATE_MEMBERS, outline_container.is_hide_private_members
	)
	...
	# --- Tabs - Start --- #
	old_scripts_tab_container = find_or_null(
	...
	multiline_tab_bar.show_close_button_always = get_setting(
		SCRIPT_TABS_CLOSE_BUTTON_ALWAYS, multiline_tab_bar.show_close_button_always
	)
	multiline_tab_bar.is_singleline_tabs = get_setting(
		SCRIPT_TABS_SINGLELINE, multiline_tab_bar.is_singleline_tabs
	)
...
func init_shortcuts():
	...
	open_quick_search_popup_gdscripts_shc = editor_settings.get_setting(
		OPEN_QUICK_SEARCH_POPUP_GDSCRIPTS
	)
	open_quick_search_popup_resources_shc = editor_settings.get_setting(
...
func open_quick_search_popup(category: QuickOpenPopup.Category = QuickOpenPopup.Category.ALL):
	...
	quick_open_popup.popup_exclusive_on_parent(
		EditorInterface.get_script_editor(), get_center_editor_rect(pref_size)
	)
...
func open_override_popup():
	...
	override_popup.popup_exclusive_on_parent(
		EditorInterface.get_script_editor(), get_center_editor_rect(pref_size)
	)
	...
	for setting: String in changed_settings:
		...
		match setting:
			...
			SCRIPT_TABS_POSITION_TOP:
				var new_script_tabs_top: bool = get_setting(
					SCRIPT_TABS_POSITION_TOP, is_script_tabs_top
				)
			...
			HIDE_PRIVATE_MEMBERS:
				var new_value: bool = get_setting(
					HIDE_PRIVATE_MEMBERS, outline_container.is_hide_private_members
				)
			...
			OPEN_QUICK_SEARCH_POPUP_GDSCRIPTS:
				open_quick_search_popup_gdscripts_shc = get_shortcut(
					OPEN_QUICK_SEARCH_POPUP_GDSCRIPTS
				)
			OPEN_QUICK_SEARCH_POPUP_RESOURCES:
				open_quick_search_popup_resources_shc = get_shortcut(
					OPEN_QUICK_SEARCH_POPUP_RESOURCES
				)
			OPEN_OVERRIDE_POPUP:
...
func update_tabs_position():
	...
	if is_script_tabs_top:
		...
		tab_container_parent.move_child(
			multiline_tab_bar, tab_container_parent.get_child_count() - 1
		)
...
func update_outline_font():
	...
	if monospace_outline:
		var font: Font = EditorInterface.get_script_editor().get_theme_font(
			&"source", &"EditorFonts"
		)
		print(font)
