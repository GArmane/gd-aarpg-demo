...
func _on_node_signal_data_requested(prefix, data) -> bool:
	...
	if target_node != null:
		if target_node != new_target_node:
			for signal_name in target_node.get_signal_list().map(
				func(p_signal): return p_signal["name"]
			):
	...
	for i in range(target_node_signal_list.size()):
		...
		var raw_signal_connections: Array[Dictionary] = target_node.get_signal_connection_list(
			raw_signal_data["name"]
		)
		...
		var parsed_signal_callables = parse_signal_callables_to_debugger_format(
			raw_signal_connections
		)

		# Create debugger-friendly signal data dictionary
		var parsed_signal_data: Dictionary = {
			"signal": parsed_signal_name, "callables": parsed_signal_callables
		}
		...
		if not target_node.is_connected(parsed_signal_name, _on_target_node_signal_emitted):
			...
			if signal_args.size() > 0:
				target_node.connect(
					parsed_signal_name,
					(
						_on_target_node_signal_emitted
						. bind(target_node_name, parsed_signal_name)
						. unbind(signal_args.size())
					)
				)
			else:
				target_node.connect(
					parsed_signal_name,
					_on_target_node_signal_emitted.bind(target_node_name, parsed_signal_name)
				)

	# On node data ready, prepare the array as per the debugger's specifications
	EngineDebugger.send_message(
		"signal_lens:incoming_node_signal_data",
		[target_node_name, target_node_signal_data, target_node_class]
	)
	return true
...
func parse_signal_callables_to_debugger_format(raw_signal_connections):
	...
	for raw_signal_connection: Dictionary in raw_signal_connections:
		...
		if not parsed_callable_object:
			return {"object_name": "ERROR: Couldn't parse node name."}
		...
		if parsed_callable_method_name == "_on_target_node_signal_emitted":
			continue

		var parsed_callable_data = {
			"object_name": parsed_callable_object_name, "method_name": parsed_callable_method_name
		}
