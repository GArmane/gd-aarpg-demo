...
enum Options {
	HIDE_SIGNALS_WITHOUT_CONNECTIONS, HIDE_BUILT_IN_SIGNALS, SHOW_GRAPH_TOOLBAR, SHOW_GRAPH_MINIMAP
}
...
func _ready() -> void:
	...
	options_button.icon = EditorInterface.get_base_control().get_theme_icon(
		"GuiTabMenuHl", "EditorIcons"
	)
	...
	keep_emissions_checkbox.icon = EditorInterface.get_base_control().get_theme_icon(
		"Override", "EditorIcons"
	)
	emission_speed_icon.icon = EditorInterface.get_base_control().get_theme_icon(
		"Timer", "EditorIcons"
	)
	logger_button.icon = EditorInterface.get_base_control().get_theme_icon(
		"FileList", "EditorIcons"
	)
	options_popup.index_pressed.connect(_on_options_index_pressed)  # NOTE: ID & index must have same value!
	connection_opacity_icon.icon = EditorInterface.get_base_control().get_theme_icon(
	...
	#graph_edit.get_menu_hbox().hide()
	repo_button.icon = EditorInterface.get_base_control().get_theme_icon(
...
func draw_node_data(data: Array):
	...
	for signal_data in target_node_signal_data:
		...
		if (
			settings[Options.HIDE_SIGNALS_WITHOUT_CONNECTIONS]
			and signal_data["callables"].size() == 0
		):
			continue
			...
			if graph_edit.has_node(object_name):
				...
				if callable_node.name == target_node.name:
					...
					if !graph_edit.has_node(target_node_name + " (Callables)"):
						...
						target_callables_node = graph_edit.get_node(
							target_node_name + " (Callables)"
						)
					create_button_slot(
						callable_method, target_callables_node, Direction.LEFT, slot_color
					)
					graph_edit.connect_node(
						target_node.name,
						current_signal_index,
						target_callables_node.name,
						target_callables_node.get_child_count() - 1
					)
				else:
					...
					graph_edit.connect_node(
						...
						current_signal_index,
						callable_node.name,
						callable_node.get_child_count() - 1
					)
			else:
				...
				# TODO: This could be a user setting
				callable_node.position_offset += Vector2(
					callable_node.get_index() * 250, callable_node.get_index() * 50
				)
				graph_edit.connect_node(
					target_node.name,
					current_signal_index,
...
func create_button_slot(
	button_text: String, parent_node: GraphNode, slot_direction: Direction, slot_color: Color
):
	...
	signal_button.pressed.connect(
		_on_signal_button_pressed.bind(parent_node, signal_button.get_index())
	)
	signal_button.focus_exited.connect(clean_connection_activity)
	parent_node.set_slot(
		signal_button.get_index(),
		slot_direction == Direction.LEFT,
		0,
		slot_color,
		slot_direction == Direction.RIGHT,
		0,
		slot_color
	)
...
func draw_signal_emission(data: Array):
	logger.create_log(
		...
		data[0]["node_name"],
		data[0]["signal_name"],
		data[0]["signal_arguments"],
		data[0]["physics_frames"],
		data[0]["process_frames"]
	)
	# Avoid trying to draw signal emission if graph not fully drawn yet
	if graph_edit.get_child_count() <= 1:
		return
...
func pulse_connection(connection: Dictionary) -> void:
	if connection not in pulsing_connections:
		pulsing_connections.append(connection)
...
func fade_out_connection(connection: Dictionary):
	...
	(
		...
		. tween_method(
			func(value):
				graph_edit.set_connection_activity(from_node, from_port, to_node, to_port, value),
			1.0,
			0.0,
			fade_out_duration
		)
...
func _get_parent_editor_split():
	...
	while not waiting.is_empty():
		...
		if node.name.find("DockVSplitCenter") >= 0:
			...
			if visible:
				_resize_panel(
					-ProjectSettings.get_setting("addons/Signal Lens/height_to_resize_to")
				)
			else:
				_resize_panel(0)
		else:
...
func _can_resize_panel() -> bool:
	...
	if not ProjectSettings.get_setting("addons/Signal Lens/resize_panel_on_open"):
		return false
...
func _on_signal_button_pressed(graph_node: GraphNode, internal_index: int):
	...
	for connection in graph_edit.get_connection_list():
		if (
			(
				connection["from_node"] == graph_node.name
				&& connection["from_port"] == internal_index
			)
			or (connection["to_node"] == graph_node.name && connection["to_port"] == internal_index)
		):
			graph_edit.set_connection_activity(
				connection["from_node"],
				connection["from_port"],
...
func _on_options_index_pressed(option_index: int) -> void:
	if options_popup.is_item_checkable(option_index):
		...
		if (
			option_index
			in [Options.HIDE_SIGNALS_WITHOUT_CONNECTIONS, Options.HIDE_BUILT_IN_SIGNALS]
		):
...
func _on_connection_opacity_slider_value_changed(value: float) -> void:
	...
	for slot_index in target_node.get_child_count():
		target_node.set_slot_color_right(
			slot_index, Color(target_node.get_slot_color_right(slot_index), connection_opacity)
		)
