...
enum Options { DATETIME, TIMESTAMP, PROCESS_FRAME, PHYSICS_FRAME }
...
func _ready() -> void:
	copy_button.icon = EditorInterface.get_base_control().get_theme_icon(
		"ActionCopy", "EditorIcons"
	)
...
func create_log(
	...
	var raw_log = (
		"%s | %s\n%s | %s\n%s → %s"
		% [datetime, timestamp, process_frames, physics_frames, node_name, signal_name]
	)
	if not signal_arguments.is_empty():
	...
	if not signal_arguments.is_empty():
		pretty_log += "[color=WHITE]" + str(signal_arguments)
